...
- [x] Window lifecycle events (emitted automatically):
  ...
  - [x] `window:focused` — `{ label }` — broadcast when a window gains focus
...
- [x] JS `fetchBuffer()` detects native vs browser mode; uses `anyar-shm://` in native, HTTP GET in browser
- [x] Existing WebSocket binary push (`ws->write_basic_buffer()`) continues to work as-is
...
- [x] Fallback test: C++ HTTP GET endpoint returns correct binary data + 404 for unknown buffers
- [x] Fallback test: JS `fetchBuffer()` uses HTTP URL in browser mode, `anyar-shm://` in native mode
...
## Phase 4g: Pinhole (Native Overlay) Rendering

> **Goal**: Provide a "punch-hole" / native overlay rendering API where C++ draws directly onto a GPU surface positioned over a designated DOM element of the webview. Eliminates the SharedBuffer copy and WebGL upload steps for the highest-throughput, lowest-latency frame path. Coexists in parallel with `@libanyar/api/canvas` (WebGL+SharedBuffer remains the portable path; pinhole is the zero-copy path).
>
> **Naming**: industry term is "punch-hole" or "platform view" (Flutter). LibAnyar adopts **`pinhole`** — distinctive, accurate metaphor.
>
> **Slicing**: Linux first (Phase 4g.1–4g.7), Windows second (Phase 7.1 additions), macOS third (Phase 7.2 additions).

### Context & Motivation

Even with Phase 4f's shared memory + WebGL path (~1ms / 1080p), the pipeline is still:
**C++ memcpy → mmap → fetch via `anyar-shm://` → `texImage2D` → fragment shader → composite**

Pinhole collapses this to **C++ draws directly on a native GL/Metal/D3D surface positioned over the DOM**. Use cases:
- 4K/8K video playback (frame budget at 60fps = 16.6ms; pinhole keeps headroom)
- Real-time camera/sensor preview
- LiDAR / point clouds (millions of points/frame)
- Game-like rendering inside a desktop app shell
- Any pipeline where the existing WebGL path's GPU memory doubling is unacceptable

| Stage | WebGL+SharedBuffer (4f) | Pinhole (4g) |
|---|---|---|
| C++ → GPU | memcpy → mmap → fetch → upload | direct GPU draw |
| Latency (1080p60) | ~1 ms | ~0.1 ms |
| GPU memory | 2× (mmap + GL texture) | 1× |
| Throughput cap | ~120 fps | display refresh |
| Composability | Inside DOM (mix-blend-mode, filter, transform) | Flat rectangle, limited CSS |

### Industry Research

| System | Mechanism | Lessons for LibAnyar |
|---|---|---|
| Chromium video overlay | Compositor "surface overlay" + chroma-key punch-through | Chroma-key requires compositor cooperation — not viable on stock WebKit/WebView2 |
| Flutter PlatformView | Two modes: hybrid composition (correct, slow) vs virtual display (fast, glitchy on scroll) | Validates "hide-during-scroll" tradeoff |
| Electron `BrowserView` | Composited views, no per-DOM-element pinning | Confirms placeholder-div approach is the standard alternative |
| WebView2 `CoreWebView2CompositionController` | Visual hosting via DComp tree | First-class API on Windows |
| WKWebView + `CAMetalLayer` | Sibling layer in NSView hierarchy | Standard pattern on macOS |
| GtkOverlay + GtkGLArea | Built-in Linux primitive | Phase 1 baseline |
| WebView2 `CreateSharedBuffer` (Phase 4f) | Zero-copy shared memory | Complementary, not redundant — keeps the in-DOM path |
...
│  ┌─ WebView (full window) ──────────────┐ │
│  │   <header>...</header>              │ │
│  │   <div data-anyar-pinhole="cam">    │ │   ← transparent placeholder
│  │     [transparent in DOM]            │ │
│  │   </div>                            │ │
│  │   <footer>...</footer>              │ │
│  └──────────────────────────────────────┘ │
│      ▲                                    │
│      │ ◄── native overlay surface (GL/Metal/D3D)
│      │     position+size synced to placeholder rect
│      │     C++ draws directly here
└───────────────────────────────────────────┘
```

JS-side (`@libanyar/api/pinhole`):
- `MutationObserver` discovers `[data-anyar-pinhole]` divs
- `ResizeObserver` + `IntersectionObserver` track size and visibility
- `requestAnimationFrame` batches rect updates → single `invoke('pinhole:update_rect', ...)` per frame
- Element styles: `pointer-events: none` on overlay (events stay with DOM); CSS `background: transparent`

C++-side:
- `anyar::Pinhole` per-window object, identified by string `id` (matches `data-anyar-pinhole` attribute)
- `Pinhole::on_render(F)` — render callback running on the platform render context
- `Pinhole::request_redraw()` — push model, default
- Rect updates from JS dispatched to main thread via `run_on_main_thread()`

### Design Decisions (locked in)

1. **Scroll behavior — hide during scroll** *(option B)*. Pinhole hides on `scroll` events anywhere in ancestor chain; reshows on `scrollend` + RAF idle. Documents as "scroll-time visual flicker is intentional; better than visual swim."
2. **Windows hosting migration — major version bump**. Phase 7 Windows port switches to `CoreWebView2CompositionController` (visual hosting); this is documented as a breaking change, not a runtime fallback.
3. **Tier-2 native handle escape hatch — defer**. Phase 1–7 ship only Tier-1 opinionated API (`pixel_format` enum: rgba/rgb/bgra/grayscale/yuv420/nv12/nv21). Tier-2 (raw `GLuint`/`id<MTLTexture>`/`ID3D11Texture2D*`) deferred to a later phase.
4. **Naming — `pinhole`**. C++ namespace `anyar::Pinhole`, JS module `@libanyar/api/pinhole`, DOM attribute `data-anyar-pinhole`.
5. **Fallback — transparent**. When pinhole can't be created (sandboxed env, GL init failure, headless test runner), `create_pinhole(id)` succeeds and returns a `Pinhole` that internally routes to `SharedBufferPool` + `FrameRenderer` driving a `<canvas>` injected into the placeholder div. Logs a warning. Same `pixel_format` enum, same `on_render` API.
6. **Coexistence with FrameRenderer — keep parallel**. `@libanyar/api/canvas` (existing, in-DOM `<canvas>` + WebGL) and `@libanyar/api/pinhole` (new, overlay surface) are independent APIs. Different DOM models (canvas element vs placeholder div), different tradeoffs. Documented side-by-side in [docs/shared-memory-webgl.md](shared-memory-webgl.md) + new `docs/pinhole-rendering.md`.

### Limitations (documented contract)

The native overlay is a flat rectangle; the following CSS does **not** affect what is rendered (overlay ignores them):
- `border-radius` (Phase 5+ may add GL stencil approximation)
- `transform: rotate / scale / 3D` on the placeholder or any ancestor → console warning, fallback engaged
- `opacity`, `mix-blend-mode`, `filter`, `backdrop-filter`
- Higher-`z-index` siblings will be visually occluded by the overlay → JS detects intersection and signals C++ to hide
- `position: sticky` ancestors → not officially supported

If any of these are needed, use `@libanyar/api/canvas` (Phase 4f path) instead.

### Performance Targets

| Metric | WebGL+SharedBuffer (4f) | Pinhole (4g target) |
|---|---|---|
| 1080p RGBA, 60fps | ~1 ms | **<0.2 ms** |
| 4K RGBA, 60fps | ~3 ms | **<0.5 ms** |
| GPU memory (1080p) | ~16 MB | **~8 MB** |
| Frame-to-screen latency | ~1 frame + composite | **1 frame** |

---

### 4g.1 Linux Prototype — Public API & GtkOverlay Foundation

> **This is "Phase 1 of Pinhole." See detailed breakdown below the section list.**

- [x] ADR-008 in [docs/decisions.md](decisions.md) — pinhole rationale, scroll-hide tradeoff, parallel-with-canvas decision
- [x] Public header `core/include/anyar/pinhole.h` — platform-neutral API
- [x] `core/src/pinhole_linux.cpp` — `GtkOverlay` + `GtkGLArea` integration
- [x] Window restructure — wrap `WebKitWebView` widget in `GtkOverlay`; existing single-widget assumption removed
- [x] Render callback context — GL context bound on main thread, viewport set to overlay size in device pixels
- [x] First end-to-end: animated gradient drawn into a 400×300 div (no JS protocol yet — fixed rect)

### 4g.2 JS-Side Rect Tracking Protocol

- [x] `js-bridge/src/modules/pinhole.ts` — `@libanyar/api/pinhole`
- [x] Auto-discovery via `MutationObserver` for `[data-anyar-pinhole]`
- [x] `ResizeObserver` per element → batched rect updates
- [x] `IntersectionObserver` → visibility events
- [x] `scroll` event listeners on document + ancestors → hide overlay; `scrollend` + RAF → reshow
- [x] DPI handling: `getBoundingClientRect()` × `window.devicePixelRatio`
- [x] IPC commands: `pinhole:update_rect`, `pinhole:set_visible`, `pinhole:get_metrics`
- [x] Public TS API: `onPinholeMounted(id, fn)`, `updatePinholeRect()`, `setPinholeVisible()`, `getPinholeMetrics()` — tracking injected as self-contained JS bootstrap via `webview_init`

### 4g.3 Pixel Formats & Render Helpers

- [x] `anyar::pixel_format` enum — re-uses `FrameRenderer`'s set: `rgba, rgb, bgra, grayscale, yuv420, nv12, nv21`
- [x] `PinholeRenderContext::draw_image(data, size, w, h, format)` — single-plane (rgba/rgb/bgra/grayscale) + multi-plane BT.601 YUV (yuv420/nv12/nv21) with dedicated GLSL program
- [x] `PinholeRenderContext::clear(color)`, `size_px()`, `dpr()`
- [x] Continuous mode flag (`set_continuous(true)`) — overlay drives RAF-equivalent (`gtk_gl_area_queue_render` per frame tick)
...
- [x] Window minimize/restore → render paused/resumed via `GDK_WINDOW_STATE_ICONIFIED` → `Pinhole::set_window_active(bool)`
- [x] Z-order: explicit `Pinhole::set_z_index(int)` for multi-pinhole windows; default = creation order; GTK reorder via `reorder_in_overlay()`
- [x] Higher-`z-index` DOM sibling intersection → JS `checkZSiblings()` signals hide (best-effort)

### 4g.5 Transparent Fallback to SharedBuffer + FrameRenderer

- [x] On GL init failure (`on_realize()` error or shader link failure), construct a fallback `Pinhole` that:
  - Allocates a `SharedBuffer` of RGBA size matching the placeholder rect
  - Injects a `<canvas>` into the placeholder div via `webview_eval` JS (self-contained IIFE; no ES module imports)
  - Wires `Pinhole::on_render` callback to write CPU-converted pixels to SharedBuffer, then calls `window.__anyar_pb_frame(id)` to drive `putImageData` on the canvas 2D context
- [x] Same `pixel_format`, same `on_render` / `draw_image` / `clear` signature → user code unchanged
  - CPU BT.601 full-range conversion for yuv420 / nv12 / nv21; channel-swap for bgra; expand for rgb / grayscale
- [x] `g_warning()` on GL failure; `Pinhole::is_native()` returns `false`
- [x] `FallbackState` shared_ptr safe for cross-thread `~Impl()` (non-main-thread destruction path)
- [x] `set_visible()` toggles injected canvas `display` style via eval
- [x] Catch2 tests: `set_eval_fn` JS capture test; BT.601 CPU-conversion spot-check; all 57 assertions pass

### 4g.6 ADR-007 Shutdown Integration

- [x] Pinhole destruction MUST precede `webview_destroy()` (mirrors window-close ordering)
- [x] GL context destroy on main thread via `g_idle_add()` (non-fiber-safe; `~Impl` extracts handles, dispatches via idle)
- [x] All render-callback fibers cancelled before service stop (documented in `on_render` docstring — contract on caller)
- [x] Add to ADR-007 shutdown sequence: "5b. Destroy all pinholes before each window's webview_destroy"
- [x] Catch2 test: create pinholes → register callbacks → explicit `.reset()` ordering → verify no crash (asan-ready: run with `-fsanitize=address`)
...
### Phase 4g Deliverable
> Linux apps can declare a `<div data-anyar-pinhole="id">` and have C++ draw directly onto a GPU overlay surface positioned over that div. ~5–10× lower latency than WebGL+SharedBuffer for high-frame-rate workloads. Coexists with `@libanyar/api/canvas` for in-DOM rendering. Transparent fallback when pinhole isn't available. Windows + macOS implementations deferred to Phase 7.

### Implementation Order (Linux)

1. **4g.1** — public API + GtkOverlay scaffold + fixed-rect gradient (Phase 1 prototype, see breakdown below)
2. **4g.2** — JS protocol for rect tracking
3. **4g.3** — pixel formats + shader port
4. **4g.4** — multi-pinhole + lifecycle
5. **4g.6** — shutdown integration + tests
6. **4g.5** — transparent fallback
7. **4g.7** — example + docs
...
- [x] Optionally embed frontend into binary (cmrc)
...
- [ ] Add `core/src/shared_buffer_macos.cpp` — POSIX `shm_open` + `WKURLSchemeHandler`
- [ ] **Pinhole (Phase 4g) macOS port**:
  - [ ] Add `core/src/pinhole_macos.mm` — sibling `CAMetalLayer` in WKWebView's hosting `NSView`
  - [ ] MSL shader port from GLSL counterparts
  - [ ] `CATransaction` per-frame rect sync
  - [ ] Validate Apple Silicon + Retina scaling correctness
...
| **7** | ~~**SharedBuffer HTTP fallback**~~ | 4f.5 | ~~1-2d~~ | ✅ Done — HTTP GET `/__anyar__/buffer/<name>` endpoint; JS `fetchBuffer()` auto-detects native vs browser mode. C++ test (260 assertions), 4 new JS tests. |
| **8** | ~~**`window:focused` event**~~ | 4d.8 | ~~0.5d~~ | ✅ Done — GTK `focus-in-event` signal → `window:focused` event. JS: `onWindowFocused()`. |
...
|---|------|-------|--------|-------|
| **9** | **C++ watch mode** (`anyar dev --watch`) | 5.2 | 1-2d | DX improvement — auto-rebuild C++ on save. |
